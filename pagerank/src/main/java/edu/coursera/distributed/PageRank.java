...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> sitesRankContributions = computeRankContributions(sites, ranks);

        return sitesRankContributions.reduceByKey(Double::sum)
                .mapValues(rank -> rank * 0.85 + 0.15);
    }

    private static JavaPairRDD<Integer, Double> computeRankContributions(JavaPairRDD<Integer, Website> sites,
                                                                         JavaPairRDD<Integer, Double> ranks) {
        return sites.join(ranks)
                .flatMapToPair(kv -> {
                    Iterator<Integer> outLinks = kv._2._1.edgeIterator();

                    List<Tuple2<Integer, Double>> outLinksContribution = new ArrayList<>();
                    while (outLinks.hasNext()) {
                        outLinksContribution.add(new Tuple2<>(outLinks.next(), kv._2._2 / kv._2._1.getNEdges()));
                    }
                    return outLinksContribution;
                });
    }
}
